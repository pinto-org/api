...
class DefaultGaugePointFunction {
  ...
  static next(currentGaugePoints, optimalPercentDepositedBdv, percentOfDepositedBdv) {
    if (percentOfDepositedBdv > DefaultGaugePointFunction._getRelativelyCloseAbove(optimalPercentDepositedBdv)) {
    ...
    } else if (percentOfDepositedBdv < DefaultGaugePointFunction._getRelativelyCloseBelow(optimalPercentDepositedBdv)) {
      const deltaPoints = DefaultGaugePointFunction._getDeltaPoints(
        optimalPercentDepositedBdv,
        percentOfDepositedBdv,
        false
  ...
  static _getRelativelyCloseAbove(optimalPercentBdv) {
    return ((MAX_PERCENT - optimalPercentBdv) * RELATIVELY_CLOSE) / PERCENT_PRECISION + optimalPercentBdv;
  }
  ...
  static _getRelativelyCloseBelow(optimalPercentBdv) {
    return (optimalPercentBdv * (PERCENT_PRECISION - RELATIVELY_CLOSE)) / PERCENT_PRECISION;
  }
}
